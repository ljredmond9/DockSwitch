...
- **Bluetooth switching**: IOBluetooth framework directly (no external dependencies)
  - `IOBluetoothDevice.remove()` — private API for removing pairing records (stable since macOS 10.6)
...
- The `remove()` method on `IOBluetoothDevice` is a private API but has been stable across all macOS versions since 10.6; it is the same call that `blueutil --unpair` used internally
